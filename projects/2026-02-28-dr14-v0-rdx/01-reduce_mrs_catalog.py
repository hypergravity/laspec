...
if hostname in ["alpha", "beta", "gamma"]:
    WORKDIR = "/nfsdata/share/lamost/dr14-v0-mrs-rdx"
    SPECDIR_1 = "/nfsdata/share/lamost/dr12-v1.1/medfits"
    SPECDIR_2 = "/nfsdata/share/lamost/dr13-v0/medfits"
else:
    raise ValueError(f"Invalid hostname {hostname}")
os.chdir(WORKDIR)

# ========================================
# Concatenate DR11 and DR12
# ========================================
cat_path_1 = "../dr12-v1.1/catalog/dr12_v1.1_MRS_catalogue.fits.gz"
cat_path_2 = "../dr13-v0/catalog/dr13_v0_MRS_catalogue_q1q2q3.fits.gz"
dr_as_int_1 = int(os.path.basename(cat_path_1).split("_")[0][2:])
dr_as_int_2 = int(os.path.basename(cat_path_2).split("_")[0][2:])
t1 = table.Table.read(cat_path_1)
t2 = table.Table.read(cat_path_2)
t1.add_column(table.Column(np.ones(len(t1), dtype=np.int16) * dr_as_int_1, name="dr"))
t2.add_column(table.Column(np.ones(len(t2), dtype=np.int16) * dr_as_int_2, name="dr"))

# remove some columns to keep colnames the same
# assert set(t2.colnames).difference(set(t1.colnames)) == set()
# colnames_diff = set(t1.colnames).difference(set(t2.colnames))
# print(colnames_diff)
#
# fill_values = {
#     "bad_b": -1,
...
# }
#
# for colname in colnames_diff:
#     # print(colname, t1[colname].dtype.name, t1[colname][:3])
#     print(f"Add column {colname} to t2")
#     t2.add_column(
#         table.Column(
#             np.full(
#                 len(t2), fill_value=fill_values[colname], dtype=t1[colname].dtype
#             ),
#             name=colname,
#         )
#     )
...
# vstack tables
t = table.vstack((t1, t2))
idx = t["coadd"] == 0  # & (t["snr"] > 5)
t = t[idx]
t.write("./dr13v0-all.fits.gz", overwrite=True)

# %% Internal match
t_BR = internal_match(
    preprocessing(
        t, colnames_reference="../dr11v0-mrs-reduction/dr10v1.0-colnames.joblib"
    ),
    n_split=3,
)

t_BR.write("dr13v0-BR.fits.gz", overwrite=True)

# %% Cut SNR
SNR = 5
t = t_BR[(t_BR["snr_B"] > SNR) | (t_BR["snr_R"] > SNR)]
print(f"{len(t_BR)}->{len(t)}")
t.write(f"dr13v0-BR-snr{SNR:d}.fits.gz", overwrite=True)
...
t_BR_im.write("dr13v0-BR-snr5-im.fits.gz", overwrite=True)
print(f"# all spectra                 = {len(t_BR)}")
print(f"# spectra with SNR>5          = {len(t)}")
print(f"# spectra with SNR>5 grouped) = {len(t_BR_im)}")

# all spectra                 = 25659427
# spectra with SNR>5          = 18067651
# spectra with SNR>5 grouped) = 18067651

# %% Generate filepath
filepath = []
for i in range(len(t_BR_im)):
    ...
    this_fp = os.path.join(
        SPECDIR_1 if this_dr == 12 else SPECDIR_2,
        this_date,
        this_planid,
        f"med-{this_lmjd}-{this_planid}_sp{this_spid:02d}-{this_fiberid:03d}.fits.gz",
    )
    filepath.append(this_fp)
    if i % 10000 == 0:
        print(f"{i}/{len(t_BR_im)}")

import joblib

existence = joblib.Parallel(n_jobs=-1, batch_size=10000, verbose=100)(
    joblib.delayed(os.path.exists)(this_fp) for this_fp in filepath
)
assert sum(existence) == len(existence)
t_BR_im.add_column(table.Column(filepath, name="fp"))
t_BR_im.write(f"dr13v0-BR-snr{SNR}-im-fp.fits.gz", overwrite=True)

# %% deprecated below -------------------
t_upload = table.Table.read("dr12v0-BR-snr5-im[uploaded_v1].fits")
