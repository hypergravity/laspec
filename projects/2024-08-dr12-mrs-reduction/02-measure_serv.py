"""
Aim:
    DR11 v1.0 + DR12 v0 RV measurements.
...
if hostname in ["alpha", "beta", "gamma", "pve-001", "pve-002"]:
...
for i_idx, idx in enumerate(idx_list):
    print(f"Prepare for {i_idx} / {len(idx_list)}")
...
n_task = 15045
# alpha
results = joblib.Parallel(n_jobs=60, verbose=99)(
    joblib.delayed(batch_processing)(i_idx) for i_idx in range(0, n_task, 1)
)

# beta
results = joblib.Parallel(n_jobs=20, verbose=99)(
    joblib.delayed(batch_processing)(i_idx) for i_idx in range(n_task - 1, 0, -1)
)

# gamma
results = joblib.Parallel(n_jobs=20, verbose=99)(
    joblib.delayed(batch_processing)(i_idx) for i_idx in range(10000, n_task, 1)
)

# pve-001
results = joblib.Parallel(n_jobs=20, verbose=99)(
    joblib.delayed(batch_processing)(i_idx) for i_idx in range(6000, 8000, 1)
)

# pve-002
results = joblib.Parallel(n_jobs=20, verbose=99)(
    joblib.delayed(batch_processing)(i_idx) for i_idx in range(8000, 10000, 1)
)


# =====================================
# process meta - should be insert into RVM.mrsbatch!
# =====================================
