...
"""
t1 = table.Table.read("../dr11-v1.0/catalog/dr11_v1.0_MRS_catalogue.fits.gz")
t2 = table.Table.read("../dr12-v0/catalog/dr12_v0_LRS_catalogue_q1q2q3.fits.gz ")
...
# ========================================
t11 = table.Table.read("../dr11-v1.0/catalog/dr11_v1.0_MRS_catalogue.fits.gz")
t12 = table.Table.read("../dr12-v0/catalog/dr12_v0_MRS_catalogue_q1q2q3.fits.gz")
t11.add_column(table.Column(np.ones(len(t11), dtype=np.int16) * 11, name="dr"))
t12.add_column(table.Column(np.ones(len(t12), dtype=np.int16) * 12, name="dr"))

# # make some columnsremove some columns to keep colnames the same
# assert set(t12.colnames).difference(set(t11.colnames)) == set()
# colnames_diff = set(t11.colnames).difference(set(t12.colnames))
# print(colnames_diff)
#
# fill_values = {
#     "bad_b": -1,
...
#     # print(colname, t11[colname].dtype.name, t11[colname][:3])
#     print(f"Add column {colname} to t12")
#     t12.add_column(
#         table.Column(
#             np.full(
#                 len(t12), fill_value=fill_values[colname], dtype=t11[colname].dtype
#             ),
#             name=colname,
#         )
#     )
...
# vstack tables
t = table.vstack((t11, t12))
idx = t["coadd"] == 0  # & (t["snr"] > 5)
t = t[idx]
t.write("./dr12v0-all.fits", overwrite=True)
...
t_BR.write("dr12v0-BR.fits", overwrite=True)

# %% Cut SNR
SNR = 5
t = t_BR[(t_BR["snr_B"] > SNR) | (t_BR["snr_R"] > SNR)]
print(f"{len(t_BR)}->{len(t)}")
t.write(f"dr12v0-BR-snr{SNR:d}.fits", overwrite=True)
...
t_BR_im.write("dr12v0-BR-snr5-im.fits", overwrite=True)
print(f"# all spectra                 = {len(t_BR)}")
print(f"# spectra with SNR>5          = {len(t)}")
print(f"# spectra with SNR>5 grouped) = {len(t_BR_im)}")

# all spectra                 = 22146899
# spectra with SNR>5          = 15044563
# spectra with SNR>5 grouped) = 15044563
...
for i in range(len(t_BR_im)):
    ...
    this_fp = os.path.join(
        SPECDIR11 if this_dr == 11 else SPECDIR12,
        this_date,
        this_planid,
...
t_BR_im.write("dr12v0-BR-snr5-im-fp.fits", overwrite=True)
